...
import { IntlMessageformatTranspiler } from '@ngneat/transloco-messageformat';

describe('IntlMessageFormatTranspiler', () => {
  const config = {};
  const parser = new IntlMessageformatTranspiler(config);
  const parserWithCustomInterpolation = new IntlMessageformatTranspiler(
    config,
